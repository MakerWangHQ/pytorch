...
# 网络参数保存文件名
PARAS_FN = 'cifar_lenet_params.pth'
...
def net_train(net, train_data_load, optimizer, epoch, log_interval):
    ...
    for i, (img, label) in enumerate(train_data_load, 0):
        ...
        if (i + 1) % log_interval == 0:
            ...
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f} Acc: {:.6f}'.format(epoch, trained_total, total, progress, loss_mean, acc))
...
def main():
    ...
    global best_acc
    print(
        'CIFAR10 pytorch VGG19Net Train: EPOCH:{}, BATCH_SZ:{}, LR:{}, ACC:{}'.format(args.epochs, args.batch_size, args.lr, best_acc))
